...
function runOperation(db, op, payload = {}) {
  switch (op) {
    ...
    case 'updateCompanyProfile': {
      ...
      if (idx < 0) return { data: null, error: { message: 'Empresa n�o encontrada' } };
    ...
    case 'updateCompanyVacancy': {
      ...
      if (idx < 0) return { data: null, error: { message: 'Vaga n�o encontrada' } };
    ...
    case 'updateApplicationStatus': {
      ...
      if (idx < 0) return { data: null, error: { message: 'Candidatura n�o encontrada' } };
      db.applications[idx].status = payload.novoStatus;
      return { data: mapApplication(db.applications[idx]), error: null };
    }

    default:
      return { data: null, error: { message: `Opera��o desconhecida: ${op}` } };
  }
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method !== 'POST') {
    return res.status(405).json({ error: { message: 'M�todo n�o permitido' } });
  }

  try {
    const { op, payload } = req.body || {};
    if (!op) {
      return res.status(400).json({ error: { message: 'Opera��o n�o informada' } });
    }
    ...
    return res.status(500).json({
      error: {
        message: 'Banco indispon�vel. Conecte Redis/KV em Vercel Storage e redeploy o projeto.',
      },
    });
  }
};
